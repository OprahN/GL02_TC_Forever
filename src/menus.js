...
import {
  saveVCardFromUserInput,
  searchAndDisplayContactInfo,
} from "./Vcard/vcard.js";
import fs from "fs";
import { search, getAllTestNames, getQuestions } from "./GIFT/search.js";
import {
  Exam,
  saveTest,
  getRandomQuestions,
...
export async function get_question_menu() {
  ...
  const preInput = await inquirer.prompt({
    type: "list",
    name: "choice",
    message: "Search a specific question or view list of questions ?",
    choices: ["Search", "View"],
  });

  if (preInput.choice === "Search") {
    ...
    if (resultTitles.length === 0) {
      console.log("No questions found that match your research");
    ...
    const selectedQuestion = searchResults.find(
      (q) => q.title === userInput2.selectedQuestion
    );
    console.log(
    ...
    let questionsTitles = questions.map((q) => q.title);
    const userInput3 = await inquirer.prompt({
      type: "list",
      name: "selectedQuestion",
      message: "Select a question:",
      choices: questionsTitles,
    });

    const selectedQuestion = questions.find(
      (q) => q.title === userInput3.selectedQuestion
    );
    console.log(
      `Question: ${selectedQuestion.title} \n Body: ${processBody(
        selectedQuestion.body
  ...
  while (true) {
    ...
    if (userInput.mainOption === "Choose a Question") {
      ...
      if (remainingQuestions > 0) {
        ...
        console.log(
          `${randomQuestions.length} random questions added to the test.`
        );
      } else {
        console.log(
          "Error: Test has reached the maximum limit of 20 questions."
        ); //pareil une secu normalement ne sert a rien
      }
...
export async function create_vcard_menu() {
  const answers = await inquirer.prompt([
    ...
    { type: "input", name: "nom", message: "Entrez le nom :" },
    {
      ...
      message: "Entrez le nom de l'établissement :",
    },
    { type: "input", name: "poste", message: "Entrez le poste :" },
    {
      type: "input",
      name: "numeroTel",
      message: "Entrez le numéro de téléphone (format : +33 1 23 45 67 89) :",
    },
    ...
    { type: "input", name: "nomRue", message: "Entrez le nom de la rue :" },
    {
      type: "input",
      name: "bp",
      message: "Entrez le numéro de boîte postale :",
    },
...
export async function simulate_test_menu() {
  ...
  const selectedTest = loadedTestDatabase.find(
    (test) => test.name === userInput.selectedTest
  );
  ...
  for (const question of questions) {
    ...
    if (question.body[1].type === "INPUT") {
      ...
      const inputOptions = question.body[1].options.map((option) =>
        option.value.split("|")
      );
      ...
      const inputOptions = question.body[1].options.map(
        (option) => option.value
      );
      ...
      const matchingOptions = question.body[1].options.map(
        (option) => option.value
      );
  ...
  console.log(
    `Test Score: ${((correctAnswers / totalQuestions) * 100).toFixed(2)}%`
  );
  ...
  const answers = await inquirer.prompt([
    {
      type: "input",
      name: "prenom",
      message: "Entrez le prénom à rechercher :",
    },
...
export async function compareExams_menu() {
  ...
  const selectedTest1 = loadedTestDatabase.find(
    (test) => test.name === userInput.selectedTest1
  );
  const selectedTest2 = loadedTestDatabase.find(
    (test) => test.name === userInput2.selectedTest2
  );
